...
function labelPrompt(){
  return `Extract the nutrition facts visible in this product photo. Prefer values per 100 g or per 100 ml when shown. If only serving values are visible, use referenceBasis="serving". Do not infer missing nutrients as zero. Return null when saturated fat, trans fat, added sugars, fiber, salt, sodium, mono- or polyunsaturated fat are not explicitly declared. If a serving or unit such as a slice is shown with its weight, return unitName and unitWeight. If a discrete unit is clearly relevant (for example sliced bread) but its weight is not visible, still return the natural unit name such as "rebanada" and null weight so the app can ask the user for an average unit weight. Do not invent a unit for foods normally weighed in bulk, such as rice or potatoes. Energy is secondary; preserve it when visible. For added sugars, only return a numeric value when explicitly quantified. Use addedSugarStatus to distinguish not stated, explicitly zero, quantified, or visible ingredients that clearly indicate added sugar. Use transFatStatus similarly and never treat an omitted trans-fat line as zero. Warnings should be short and only mention ambiguity or missing critical data. Respond in Spanish for names and warnings.`
}
function mealPrompt(){
...
export default async function handler(req,res){
  ...
  try{
    const response=await fetch('https://api.openai.com/v1/responses',{
      ...
      body:JSON.stringify({
        model:String(process.env.OPENAI_NUTRITION_MODEL||'gpt-5.6-sol'),
