...
try{
 ...
 assert.match(doc.getElementById('modalRoot').textContent,/Kebab/);
 run(`openCommonFoodRegistration('Desayuno');document.getElementById('manualBarcode').value='8410000000000'`);
 ...
 run(`openManualPunctualFood('Merienda');document.getElementById('punName').value='Comida puntual test';document.getElementById('punKcal').value='500';document.getElementById('punAmount').value='1';document.getElementById('punBasis').value='serving';savePunctualProduct()`);
 assert.ok(run(`state.customFoods.some(f=>f.name==='Comida puntual test'&&f.transient)`),'punctual food is stored only as a transient diary food');
 assert.ok(!run(`allFoodKeys().some(k=>foodRecord(k)?.name==='Comida puntual test')`),'punctual food never enters reusable food search');
 run(`const row=state.foods.find(i=>foodRecord(i.foodKey)?.name==='Comida puntual test');if(row){state.foods=state.foods.filter(i=>i.id!==row.id);state.customFoods=state.customFoods.filter(f=>f.key!==row.foodKey)}`);
 ...
 const lunchChicken=run(`mealPlan('2026-09-25','Almuerzo').items.find(([k])=>k==='chicken')[1]`);
 const dinnerChicken=run(`mealPlan('2026-09-25','Cena').items.find(([k])=>k==='chicken')[1]`);
 assert.equal(lunchChicken,dinnerChicken,'the same protein food always uses one standard tupper portion');
 assert.ok(lunchChicken>=110&&lunchChicken<=280,'standard protein portion stays inside the planned safe range');
 const riceBeforeLabel=run(`tupperPortions().rice`);
 run(`state.customFoods.push({key:'custom_rice_label',name:'Mi arroz',cat:'Carbohidrato',unit:'g',ref:'etiqueta por 100 g',custom:true,kcal:650,p:7,c:145,f:1,inputMeta:{inputUnit:'g',reference:'peso en seco'}});state.nutritionPlan.preferredFoods.rice='custom_rice_label'`);
 const riceAfterLabel=run(`tupperPortions().rice`);
 assert.notEqual(riceAfterLabel,riceBeforeLabel,'real nutrition data recalculates the future standard rice portion');
 assert.ok(riceAfterLabel<riceBeforeLabel,'a more energy-dense rice produces a smaller dry portion');
 run(`delete state.nutritionPlan.preferredFoods.rice;state.customFoods=state.customFoods.filter(f=>f.key!=='custom_rice_label')`);
 const breakfastEggs=run(`mealPlan('2026-09-25','Desayuno').items.find(([k])=>k==='egg')?.[1]`);
 assert.ok(breakfastEggs>=1&&breakfastEggs<=4,'egg quantity is solved by the meal optimizer instead of being permanently fixed');
 ...
 const postBeforeSkip=run(`JSON.stringify(mealPlan('2026-09-25','Post-entreno').items)`);
 const totalBeforeSkip=run(`dayPlanNutrition('2026-09-25').kcal`);
 ...
 assert.equal(run(`mealTargetMap('2026-09-25').Merienda.kcal`),snackTargetBeforeSkip,'skipping a tupper does not redistribute calories into flexible meals');
 assert.ok(run(`dayPlanNutrition('2026-09-25').kcal`)<totalBeforeSkip,'skipping a tupper leaves the day with fewer consumed/planned calories');
 assert.match(doc.getElementById('viewFood').textContent,/No hecha/);
 run(`addSkippedTupperToPost('2026-09-25','Almuerzo')`);
