...
try{
 ...
 const dinnerBeforeSkip=run(`mealPlan('2026-09-25','Cena').nutrition.kcal`);
 run(`toggleNutritionPlanMealSkipped('2026-09-25','Almuerzo')`);
 assert.ok(run(`mealPlan('2026-09-25','Cena').nutrition.kcal`)>dinnerBeforeSkip,'skipping lunch reallocates energy to a later flexible home meal');
 ...
 assert.equal(run(`state.nutritionPlan.amountOverrides['2026-09-25'].Almuerzo.amounts[Object.keys(state.nutritionPlan.amountOverrides['2026-09-25'].Almuerzo.amounts)[0]]`),210,'day rebalance preserves the prepared lunch amount');
